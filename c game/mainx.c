...
typedef enum {
    OPENING_SCENE,
    PRE_GAME_IMAGE, // New state for showing the image
    GAMEPLAY,
    CLOSING_SCENE
} GameState;
...
Texture2D logo;
Texture2D balhTex;
Texture2D aoiTex; // New texture for pre-game image
...
int main(void) {
    ...
    const char* aoiPath = "resources/aoi.png";
    if (!IsPathFile(aoiPath)) TraceLog(LOG_WARNING, "aoi.png missing!");
    aoiTex = LoadTexture(aoiPath); // Load new image
    ...
    float imageTimer = 0; // Timer for pre-game image
    ...
    while (!WindowShouldClose()) {
        ...
        switch (gameState) {
            case OPENING_SCENE:
                ...
                if (CheckCollisionPointRec(GetMousePosition(), startBtn) && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                    ...
                    gameState = PRE_GAME_IMAGE; // Transition to new state
            ...
            case PRE_GAME_IMAGE:
                imageTimer += dt;
                if (imageTimer >= 3.0f) { // Show image for 3 seconds
                    gameState = GAMEPLAY;
                }
                break;
            ...
            case GAMEPLAY:
                if (!gameOver) {
                    ...
                    for (int i = 0; i < MAX_ENEMIES; i++) {
                        if (enemies[i].active) {
                            // Recalculate velocity to target the player's current position (homing behavior)
                            float speed = (selectedDifficulty == DIFFICULTY_EASY) ? 50.0f :
                                          (selectedDifficulty == DIFFICULTY_MEDIUM) ? 60.0f : 70.0f;
                            Vector2 direction = Vector2Subtract(playerPos, enemies[i].position);
                            if (Vector2Length(direction) > 0) { // Avoid division by zero if positions overlap
                                enemies[i].velocity = Vector2Scale(Vector2Normalize(direction), speed);
                            } else {
                                enemies[i].velocity = (Vector2){0, 0}; // Stop if exactly on player (collision will handle)
                            }
        ...
        switch (gameState) {
            ...
            case PRE_GAME_IMAGE:
                DrawTexture(aoiTex, screenWidth/2 - aoiTex.width/2, screenHeight/2 - aoiTex.height/2, WHITE); // Center the image
                break;
    ...
    UnloadTexture(aoiTex); // Unload new image
    CloseWindow();
    return 0;
}